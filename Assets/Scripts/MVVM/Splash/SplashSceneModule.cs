...
namespace TJ.Decaf.MVVM.Splash
{
    ...
    public class SplashSceneModule : MonoBehaviour, IModule
    {
        ...
        public Transform RegisterModule(UINavigationManager navigationManager)
        {
            if (TryGetComponent<ViewBase>(out var view) && TryGetComponent<ViewModelBase>(out var viewModelBase))
            {
                ...
                navigationManager.RegisterForNavigation(new UINavigationManager.Page(PageConstants.SplashPage, view, viewModelBase, new UINavigationManager.Page.PageOption()
                {
                    IsForceOpenAlways = true,
                }))
                    .OnAppStart(PageConstants.SplashPage);
            }
